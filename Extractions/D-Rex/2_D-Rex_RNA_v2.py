...
def run(protocol):
    ...
    #### REAGENT SETUP
    # keep DNase on twice                     Description             Volume needed for protocol
    ...
    # to check if it more convenient to use the line below to transfer 700ul.
    # for i in list_of_cols:
    #     m300.transfer(700, RNA_plate[i].bottom(1), Liquid_trash.top(-4), new_tip='once',  blow_out =True, air_gap=30)
    ...
    protocol.pause('Please ensure enough ethanol in the reservoir by adding 3ml')
...
### probably to cancel:
    ...
    protocol.pause("Please cover the plate with foil and spin down. Incubate 10 min 25°C at 1300 rpm. \
    Please fill up tips before continuing process")
    ...
    for i in list_of_cols:
        ...
        protocol.pause("Please cover the plate with foil and spin down. Incubate 10 min 25°C at 1000 rpm. Quickly spin it down.")
    ...
    for i in list_of_cols:
        ...
        m300.flow_rate.dispense = 100
        m300.pick_up_tip(tipracks_200_1[i])
        m300.aspirate(250, RNA_plate[i].bottom(1))
        m300.dispense(250, trash_box['A1']).top(-5))
    ...
    protocol.pause('If beads are not dried, let them dry for 5 minutes')
    ...
    robot.pause("Please cover the plate with foil and incubate 5 min 25°C at 1500 rpm")
    mag_deck.engage(height=34)
    m300.delay(minutes=2)
    ...
    protocol.pause('Please cover plate with foil and store at -80°C until further process')
